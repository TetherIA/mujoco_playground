...
def step_response(model, data, viewer, tendon_name, settle_time, sim_time, edge_margin):
    ...
    for tendon_name in TENDON_NAMES:
        tendon_id_temp = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_TENDON, tendon_name)
        if tendon_id_temp == -1:
            continue
        actuator_id_temp = find_position_actuator_for_tendon(model, tendon_id_temp)
        if actuator_id_temp is None:
            continue
        lo_all, hi_all = model.actuator_ctrlrange[actuator_id_temp]
        span_all = hi_all - lo_all
        hi_eff_all = hi_all
        data.ctrl[actuator_id_temp] = float(hi_eff_all)
    ...
    #data.ctrl[actuator_id] = MOTOR_LIMIT[tendon_name][0]+0.069
    #data.ctrl[aid] = 0.07
    ...
    t_list, y_list = [], []
    start = time.time()
    print(f"======Drive current tendon to low reference only, high: {hi}, low: {lo}=======")
...
def main():
    model = mujoco.MjModel.from_xml_path(MODEL)lo_eff
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        if curves:
            ...
            for ax, (tendon_name, t, y, sp) in zip(axes, curves):
                ...
                ax.set_title(f'{tendon_name}: min:{ymin:.2f} max:{ymax:.2f}')
