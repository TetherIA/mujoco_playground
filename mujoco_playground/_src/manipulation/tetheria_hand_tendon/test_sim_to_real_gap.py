...
def find_position_actuator_for_tendon(m, tid):
    for aid in range(m.nu):
        if (
            m.actuator_trntype[aid] != mujoco.mjtTrn.mjTRN_TENDON
        ):  # it is used to distinguish tendon actuators from other types of actuators
            continue
        if (
            m.actuator_trnid[aid, 0] != tid
        ):  # the left side gives the tendon ID that this actuator controls
        ...
        ):  # return the actuator id if all conditions are met
            return aid
    return None


def step_response_real(
    model, data, viewer, tendon_name, real_ctrl, resolution=25
):  # real_ctr is from the real motor
    ...
    for i in range(len(real_ctrl)):
        data.ctrl[actuator_id] = np.interp(
            real_ctrl[i], [real_ctrl_min, real_ctrl_max], [hi, lo]
        )  # map real to sim
        # data.ctrl[actuator_id] = hi-real_ctrl[i]  # map real to sim
        for _ in range(
            resolution
        ):  # this can solve the problem of different sampling frequency between sim (0.01) and real (0.2)
            mujoco.mj_step(
                model, data
            )  # the control signal is processed during each simulation step
        viewer.sync()
...
def step_response(
    model, data, viewer, tendon_name, data_len=5, resolution=25, edge_margin=0.1
):
    tendon_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_TENDON, tendon_name)
    if tendon_id == -1:
        raise RuntimeError(f"Tendon {tendon_name} not found")
...
def main(DATA_PATH):
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        for tendon_name in TENDON_NAMES_ID_MAP.keys():
            ...
            y1 = step_response_real(
                model, data, viewer, tendon_name, real_ctrl[f"tendon_{tendon_id}_sent"]
            )  # read/sent, real means control from the real motor
            y2 = step_response(
                model,
                data,
                viewer,
        ...
        if curves:
            ...
            for ax, (tendon_name, y1, y2) in zip(axes, curves):
                ...
                ax.set_title(
                    f"{tendon_name}: real:({y1_min:.2f}, {y1_max:.2f}), sim:({y2_min:.2f}, {y2_max:.2f})"
                )
